...
from Efetch_cblaster3 import run_fetch_nucl_cblaster, run_fetch_nucl  # 从 Efetch2.py 导入 run_fetch_data 函数
...
def main():
    ...
    if args.nucl_file:
        ...
        UDlength = args.updown_length
        Nuclfile = run_fetch_nucl_cblaster(input_file, UDlength)
    ...
    if not args.nucl_file:  # 如果是新下载的，则移动（已有文件就已经复制了）
        nucl_target_path = os.path.join(out_dir, os.path.basename(Nuclfile))
        if os.path.abspath(Nuclfile) != os.path.abspath(nucl_target_path):  # 防止重复移动
            shutil.move(Nuclfile, nucl_target_path)
            Nuclfile = nucl_target_path  # 更新路径以防后续用到
...
#python3 Main_cblaster3.py --accession_file a.txt --updown_length 10000 --pre_min 20 --pre_max 150 --out_dir a_out_dir
#python3 Main_cblaster3.py --nucl_file b_20kb_nucl.fasta --updown_length 10000 --pre_min 20 --pre_max 150 --out_dir b2_out_dir
#python3 Main_cblaster3.py --cblaster_file c.txt --updown_length 10000 --pre_min 20 --pre_max 150 --out_dir c_out_dir